...
function StatusIndicator({ status }: { status: ChatSession['status'] }) {
  if (status === 'running') {
    return (
      <span className="relative flex h-2.5 w-2.5 shrink-0">
        <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-stage-active opacity-75" />
        <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-stage-active" />
      </span>
    );
  }
  if (status === 'completed_unread') {
    return <span className="w-2.5 h-2.5 rounded-full bg-stage-done shrink-0 ring-2 ring-stage-done/30" />;
  }
  return null;
}
