...
export default function ChatRail({ unreadCount, onExpand }: ChatRailProps) {
  return (
    ...
    >
      ...
      {/* Unread badge */}
      {unreadCount > 0 && (
        <span data-testid="chat-rail-unread-badge" className="bg-soul-dim text-soul text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1">
      ...
      {/* Chat bubble icon */}
      <button type="button" data-testid="chat-rail-expand-btn" className="w-7 h-7 flex items-center justify-center rounded text-fg-muted hover:text-fg hover:bg-elevated transition-colors" title="Expand chat">
