...
async def test_cluster_overview_has_role_column():
    from soul_mesh.dashboard import ClusterOverview
    from textual.app import App

    class TestApp(App):
        def compose(self):
            yield ClusterOverview()

    app = TestApp()
    async with app.run_test():
        screen = app.query_one(ClusterOverview)
        table = screen.query_one("#node-table")
        col_labels = [str(col.label) for col in table.columns.values()]
        assert "Role" in col_labels
        assert col_labels.index("Role") == 1  # between Name and Status


async def test_refresh_table_hub_role_label():
    from soul_mesh.dashboard import ClusterOverview
    from textual.app import App
    ...
    async with app.run_test():
        ...
        screen.refresh_table([
            {"id": "h1", "name": "pi", "role": "hub", "status": "online"},
            {"id": "a1", "name": "pc", "role": "agent", "status": "online"},
        ])
        table = screen.query_one("#node-table")
        # Row 0 col 1 should be "Hub", row 1 col 1 should be "Node 1"
        from textual.coordinate import Coordinate

        hub_cell = table.get_cell_at(Coordinate(0, 1))
        agent_cell = table.get_cell_at(Coordinate(1, 1))
        assert str(hub_cell) == "Hub"
        assert str(agent_cell) == "Node 1"
