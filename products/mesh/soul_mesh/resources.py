...
async def _storage_linux() -> dict:
    ...
    Tries GNU ``df -BG --output=target,size,avail`` first.  If that fails
    (e.g. Android toybox), falls back to POSIX ``df -k``.
    """
    proc = await asyncio.create_subprocess_exec(
    ...
    if proc.returncode == 0:
        return _parse_gnu_df(stdout.decode())

    # Fallback: POSIX df -k (kilobytes, universal)
    proc = await asyncio.create_subprocess_exec(
        "df", "-k",
        stdout=asyncio.subprocess.PIPE,
        stderr=asyncio.subprocess.PIPE,
    ...
    stdout, _ = await proc.communicate()
    return _parse_posix_df(stdout.decode())


def _parse_gnu_df(output: str) -> dict:
    """Parse GNU ``df -BG --output=target,size,avail`` output."""
...
def _parse_posix_df(output: str) -> dict:
    """Parse POSIX ``df -k`` output (Android/BusyBox compatible).

    Expected format::

        Filesystem     1K-blocks    Used Available Use% Mounted on
        /dev/sda1      12345678  1234567  9876543  10% /
    """
    lines = output.strip().splitlines()
    mounts: list[dict] = []
    for line in lines[1:]:
        ...
        try:
            ...
            continue
        mountpoint = parts[5]
        if mountpoint.startswith(("/dev", "/sys", "/proc", "/run", "/snap")):
            continue
        total_gb = total_kb // (1024 * 1024)
        free_gb = avail_kb // (1024 * 1024)
        mounts.append({"path": mountpoint, "total_gb": total_gb, "free_gb": free_gb})
    return {"mounts": mounts}
