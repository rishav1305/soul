...
async def _get_ram_mb() -> int:
    ...
    On Linux, falls back to ``/proc/meminfo`` if ``free`` is not installed
    (e.g. Android without the ``procps`` package).
    """
    try:
        if _platform.system().lower() == "linux":
            try:
                ...
                pass  # free not installed, fall through to /proc/meminfo

            # Fallback: /proc/meminfo (always available on Linux)
            content = await asyncio.to_thread(Path("/proc/meminfo").read_text)
            for memline in content.splitlines():
                if memline.startswith("MemTotal:"):
                    return int(memline.split()[1]) // 1024
            return 0
...
async def _get_storage_mb() -> int:
    """Total root-partition storage in MiB.

    Tries ``df -m /`` first.  If that fails (e.g. Android toybox doesn't
    support ``-m``), falls back to ``df -k /`` and converts.
    """
    try:
        proc = await asyncio.create_subprocess_exec(
            "df", "-m", "/",
